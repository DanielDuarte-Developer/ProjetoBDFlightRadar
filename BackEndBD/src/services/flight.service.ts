...
export class FlightService implements IFlightService {
  ...
  async AddAsync(item: Flight, userId: string) {
    try {
      ...
      throw new Error("Error trying to insert an Flight: ", error);
    }
  }
  ...
  async UpdateAsync(item: Flight, userId: string) {
    try {
      ...
      throw new Error("Error trying to update an Flight: ", error);
    }
  }
  ...
  async DeleteAsync(id: string, userId: string) {
    try {
      ...
      throw new Error("Error trying to delete an Flight: ", error);
    }
  }
  ...
  async GetByIdAsync(id: string): Promise<FlightDTO> {
    try {
      ...
      const observation: Observation =
        await this.observationRepository.GetAsync(flight[0].IdObservation);
      const airplane: Airplane = await this.airplaneRepository.GetAsync(
        flight[0].IdAirplane
      );
      ...
      throw new Error("Error trying to get Flight by id: ", error);
    }
  }
  ...
  async ListAsync(
    ...
    try {
      const flights: Flight[] = await this.flightRepository.ListFlights(
        idFlight,
        idObservation,
        idAirplane,
        flightCode,
      ...
      const flightDTOs = await Promise.all(
        flights[0].map(async (flight) => {
          const observation: Observation =
            await this.observationRepository.GetAsync(flight.IdObservation);
          const airplane: Airplane = await this.airplaneRepository.GetAsync(
            flight.IdAirplane
          );
          const model: Model = await this.modelRepository.GetAsync(
