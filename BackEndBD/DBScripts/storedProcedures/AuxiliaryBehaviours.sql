...
begin
    select IdAirport, TimeMarker from airport_flight where IdAirport = flightId order by TimeMarker asc limit 1;
end $$

create procedure getArrival(flightId varchar(100))
begin
    select IdAirport, TimeMarker from airport_flight where IdAirport = flightId order by TimeMarker desc limit 1;
end $$

create procedure getStopOvers(flightId varchar(100))
begin
    select IdAirport, TimeMarker from airport_flight where IdAirport = flightId and TimeMarker > (select TimeMarker from airport_flight order by TimeMarker asc limit 1) and TimeMarker < (select TimeMarker from airport_flight order by TimeMarker desc limit 1);
end $$
